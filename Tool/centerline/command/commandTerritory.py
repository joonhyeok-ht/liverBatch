...
"""
Multi Territory
- 다수의 labeling 정보를 통해 Territory를 나눈다. 
"""
class CCommandMultiTerritoryLabel(commandInterface.CCommand) :
    # @staticmethod
    ...
    #         vertex = algLinearMath.CScoMath.mul_mat4_vec3(terriInfo.InvMat, vertex)
    ...
    #     for inx, label in enumerate(listLabel) :
    #         anchorID = inx + 1
    #         listBlobCoord = CCommandMultiTerritoryLabel.get_segment_vertex_from_np_value(npImg, anchorID, np.int32)
    ...
    #     if bSecondFlag == True :
    #         boundaryCoord = CCommandMultiTerritoryLabel.get_blob_boundary(npImgTmp, 255, np.int32)
    ...
    def process(self) :
        ...
        for inx, label in enumerate(listLabel) :
            ...
            vertex = self.m_dataGroupVertex.get_vertex(label)
            # vertex = algLinearMath.CScoMath.mul_mat4_vec3(terriInfo.InvMat, vertex)
            # vertex = algLinearMath.CScoMath.mul_mat4_vec3(terriInfo.InvMat, vertex).astype(np.int32)
            vertex = np.round(algLinearMath.CScoMath.mul_mat4_vec3(terriInfo.InvMat, vertex)).astype(np.int32)
            segmentProcess.add_anchor(vertex, anchorID)
            # dicAnchor[anchorID] = vertex
            dicAnchor[anchorID] = np.round(vertex).astype(np.int32)
        segmentProcess.process(terriInfo.QueryVertex)
