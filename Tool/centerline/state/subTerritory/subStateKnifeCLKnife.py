...
class CSubStateKnifeCLKnife(subStateKnife.CSubStateKnife) :
    ...
    def process_end(self) :
        ...
        self.App.remove_key_type(CSubStateKnifeCLKnife.s_knifeKeyType)
        self.App.remove_key_type(CSubStateKnifeCLKnife.s_knifeCLKeyType)
    ...
    def clicked_mouse_rb(self, clickX, clickY) :
        ...
        worldStart, pNearStart, pFarStart= self.App.get_world_from_mouse(self.m_startMx, self.m_startMy, CSubStateKnifeCLKnife.s_pickingDepth)
        worldEnd, pNearEnd, pFarEnd = self.App.get_world_from_mouse(self.m_endMx, self.m_endMy, CSubStateKnifeCLKnife.s_pickingDepth)

        self.m_knifeKey = data.CData.make_key(CSubStateKnifeCLKnife.s_knifeKeyType, 0, 0)
        inst = vtkObjLine.CVTKObjLine()
        inst.KeyType = CSubStateKnifeCLKnife.s_knifeKeyType
        ...
        opSelectionCL.process_reset()
        self._set_whole_vessel(None)
        self.App.ref_key_type(CSubStateKnifeCLKnife.s_knifeKeyType)
        self.App.remove_key_type(CSubStateKnifeCLKnife.s_knifeCLKeyType)
        self.App.remove_key_type(data.CData.s_territoryType)
        self.m_knifeCLID = -1
        self.m_knifeInx = -1
    ...
    def release_mouse_rb(self) :
        ...
        if dist < CSubStateKnifeCLKnife.s_minDragDist :
            return
    ...
    def on_btn_view_vessel(self) :
        if self.m_knifeCLID == -1 :
            ...
            self._command_vessel_knife(self.m_knifeCLID, self.m_knifeInx, self.m_tangent)
    ...
    def _command_knife(self, startMx, startMy, endMx, endMy) :
        ...
        worldStart, pNearStart, pFarStart = self.App.get_world_from_mouse(startMx, startMy, CSubStateKnifeCLKnife.s_pickingDepth)
        worldEnd, pNearEnd, pFarEnd = self.App.get_world_from_mouse(endMx, endMy, CSubStateKnifeCLKnife.s_pickingDepth)
    ...
    def _command_territory_knife(self, knifeCLID : int, knifeInx : int, tangent : np.ndarray) :
        ...
        terriObj.Key = key
        terriObj.Color = subStateKnife.CSubStateKnife.s_terriOrganColor
        terriObj.Opacity = 0.5
        terriObj.PolyData = terriPolyData
        dataInst.add_vtk_obj(terriObj)
        self.App.ref_key(key)
        self.App.update_viewer()
    def _command_vessel_knife(self, knifeCLID : int, knifeInx : int, tangent : np.ndarray) :
        ...
        cmd = commandTerritoryVessel.CCommandTerritoryVesselKnife(self.App)
        cmd.InputData = self._get_data()
        cmd.InputSkeleton = skeleton
        cmd.InputVoxelizeSpacing = (1.0, 1.0, 1.0)
        cmd.InputKnifeCLID = knifeCLID
        cmd.InputKnifeIndex = knifeInx
        cmd.InputTangent = tangent
        for clID in selList :
            cmd.add_cl_id(clID)
        cmd.process()
        ...
        terriObj.KeyType = data.CData.s_territoryType
        terriObj.Key = key
        terriObj.Color = subStateKnife.CSubStateKnife.s_terriVesselColor
        terriObj.Opacity = 0.5
