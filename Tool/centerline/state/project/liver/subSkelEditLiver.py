...
class CSubSkelEditLiver :
    ...
    def process(self) :
        ...
        self.progress_callback(100, "Save Centerline ...")
    ...
    def check_outside_centerline_point(self, skeleton, polydata) :
        print(f"Check if the centerline points are inside the mesh ->", file=sys.__stdout__, flush=True)
        outside_list = []
        ...
        print(f"Check Done." , file=sys.__stdout__, flush=True)
        return outside_list
            ...
            if retListCenterline is not None :
                ...
                for cl in retListCenterline :
                    ...
                    if len(hierarchy[1]) == 0 : 
                        if cl.is_leaf() == False :
                            print(f"[]Info]Broken Loop Centerline(in Tree) : CL {cl.ID}" , file=sys.__stdout__, flush=True)
                            broken_loop_cl_list.append(cl.ID)  #뒷단에서 이 cl의 마지막 cp를 끊어진cl리스트에 등록하게 됨.
                    hierarchy_list.append([hierarchy[0], cl.ID])
            else :
                print(f"not found centerline in depth. Stop. depth = {depth}" , file=sys.__stdout__, flush=True)
                stopFlag = False
            depth = depth + 1
    ...
    def find_neighbors(self, polyVertex : np.ndarray, polyNormal : np.ndarray, listCenterline : list, listCenterlineByDepth : list, listBrokenLoopCL : list):
        ...
        print(f"Centerline Count : {len(listCenterline)}" , file=sys.__stdout__, flush=True)
        ...
        print(f"brokenLoopCLCPIdxList : {brokenLoopCLCPIdxList}" , file=sys.__stdout__, flush=True)    
        print(f"brokenLoopCLCPIdxAllList : {brokenLoopCLCPIdxAllList}" , file=sys.__stdout__, flush=True) 
        ...
        print("Number of Centerline Points : ", len(npCenterlinePoints) , file=sys.__stdout__, flush=True)

        print("kd-tree build start" , file=sys.__stdout__, flush=True)
        ...
        print(f"end of find_neighbors()" , file=sys.__stdout__, flush=True)
    ...
    def __get_near_lines4(self, target_vertex, neighbor_cp_indices : list, cp_all : np.ndarray, cell_info : list, try_no) -> list:
        ...
        if len(neighbor_line_info_list) < self.NEEDED_LINE_CNT:
            print(f"ERROR : Number of neighbors(LineCell) are less than {self.NEEDED_LINE_CNT}.(Try ({try_no})" , file=sys.__stdout__, flush=True)
