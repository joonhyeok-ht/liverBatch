...
class CTabStateSkelEditLiver(tabState.CTabState) :
    ...
    def __init__(self, mediator):
        ...
        self.m_opSelectionVertex = operation.COperationSelectionVertex(mediator)
        self.m_listSubState = []
    ...
    def clear(self) :
        ...
        self.m_opSelectionVertex.clear()
        self.m_opSelectionVertex = None
        self.m_state = 0
        self.m_bReady = False
        self.m_selectedCLInxList = []
        super().clear()
    ...
    def process_init(self) :
        ...
        clinfoInx = self.get_clinfo_indices()[0]
    ...
    def init_ui(self) :
        ...
        # layout, retList = self.m_mediator.create_layout_label_radio("SelectionMode", ["CL", "Br", "EP", "Merge"])
        # self.m_rbSelectionModeCL = retList[0]
        # self.m_rbSelectionModeBr = retList[1]
        # self.m_rbSelectionModeEP = retList[2]
        # self.m_rbSelectionModeVertex = retList[3]
        ...
        # self.m_rbSelectionModeVertex.toggled.connect(self._on_rb_selection_mode_vertex)
        # self.m_rbSelectionModeCL.setChecked(True)
        # tabLayout.addLayout(layout)
        
        layout, retList = self.m_mediator.create_layout_label_radio("SelectionMode", ["CL", "Br", "EP", "Vertex"])
        self.m_rbSelectionModeCL = retList[0]
        self.m_rbSelectionModeBr = retList[1]
        self.m_rbSelectionModeEP = retList[2]
        self.m_rbSelectionModeVertex = retList[3]
        ...
        layout, self.m_sliderRange, self.m_editRange = self.m_mediator.create_layout_label_slider_editbox("Range", CTabStateSkelEditLiver.s_minRange, CTabStateSkelEditLiver.s_maxRange, 1, True)
    ...
    def _on_rb_selection_mode_ep(self) :
        ...
        if self.m_rbSelectionModeEP.isChecked() :
            ...
            self._get_substate(self.m_state).process_end()
    def _on_rb_selection_mode_vertex(self) :
        if self.m_bReady == False :
            return
        
        if self.m_rbSelectionModeVertex.isChecked() :
            self.m_state = 3
            self._get_substate(self.m_state).process_init()
        else :
            self._get_substate(self.m_state).process_end()
