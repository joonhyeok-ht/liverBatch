...
class CTabStateStomachVesselKnife(tabState.CTabState) :
    def __init__(self, mediator):
        ...
        self.m_lastSelectedInvalidVessel = None
        self.m_lastSelectedNode = None
    def clear(self) :
        # input your code
    ...
    def init_ui(self) :
        ...
        label = QLabel("-- Liver Vessel Hierarchy Test --")
        ...
        btn = QPushButton("Save Selected Vessel")
        btn.setStyleSheet(self.get_btn_stylesheet())
        btn.clicked.connect(self._on_btn_save_selected_vessel)
        tabLayout.addWidget(btn)

        btn = QPushButton("Save All Valid Vessel")
        btn.setStyleSheet(self.get_btn_stylesheet())
        btn.clicked.connect(self._on_btn_save_all_valid_vessel)
        tabLayout.addWidget(btn)
    ...
    # ui setting
    def _command_save_selected_vessel(self):
        dataInst = self.get_data()
        ...
        fullPath = os.path.join(tpinfoOutPath, f"test.stl")
        validVessel = self.m_lastSelectedNode.get_valid_vessel()
        if validVessel is None :
            return 
        
        algVTK.CVTK.save_poly_data_stl(fullPath,  validVessel)
    ...
    # command
    def _command_save_all_valid_vessel(self) :
    ...
    def _on_btn_save_all_valid_vessel(self) :
        self._command_save_all_valid_vessel()
        QMessageBox.information(self.m_mediator, "Alarm", "complete to save vessel")

    def _on_btn_save_selected_vessel(self) :
        self._command_save_selected_vessel()
        #QMessageBox.information(self.m_mediator, "Alarm", "complete to save vessel")
    ...
    def __add_vessel_territory(self, node : treeVessel.CNodeVesselHier) :
        ...
        # fullPath = os.path.join(tpinfoOutPath, f"test.stl")
        # wholevessel = self.m_lastSelectedNode.get_whole_vessel()
        # if validVessel is None :
        #     return 
        
        # algVTK.CVTK.save_poly_data_stl(fullPath,  wholevessel)
        ##############################################
