...
import state.project.stomachBatch.userDataStomachBatch as userDataStomach
...
# sally
import stomachBatch.makeInputFolderStomachBatch as makeInputFolder
...
import subRecon.subReconStomach as reconStomach
import stomachBatch.subDetectOverlap.subDetectOverlapStomach as detectOverlap
...
class CTabStatePatient(tabState.CTabState):
    ...
    s_listStepName = ["IntegrityCheck", "Recon", "Overlap", "MeshClean"]
    ...
    def _get_userdata(self) -> userDataStomach.CUserDataStomach:
        return self.get_data().find_userdata(
            userDataStomach.CUserDataStomach.s_userDataKey
        )
    ...
    def _changed_input_path(self, inputPath) -> str:
        ...
        mkInputFold.FolderMode = mkInputFold.eMode_Stomach
    ...
    def do_blender(self):
        ...
        cmd = f"{dataInst.OptionInfo.BlenderExe} --python {os.path.join(self.fileAbsPath, 'blenderScriptStomach.py')} -- --patient_id {currPatientID} --stl_path {stlPath} --out_path {savePath} --func_mode Basic --option_path {self.m_optionFullPath}"
        os.system(cmd)
    ...
    def _clicked_recon_mask(self, inputSliceID: int):
        ...
        if self.m_reconReady == True:
            reconInst = reconStomach.CSubReconStomach()
    ...
    def _clicked_overlap_loading(self):
        ...
        def _clicked_overlap(self):
            ...
            cmd = f"{dataInst.OptionInfo.BlenderExe} -b --python {os.path.join(self.fileAbsPath, 'blenderScriptStomach.py')} --\
            ...
            cmd = f"{dataInst.OptionInfo.BlenderExe} --python {os.path.join(self.fileAbsPath, 'blenderScriptStomach.py')} -- \
    ...
    def _init_recon_stomach(self) -> bool:
        if self.m_reconStomach == None:
            self.m_reconStomach = reconStomach.CSubReconStomach()
        self.m_reconStomach.OptionPath = self.m_optionFullPath
        self.m_reconStomach.PatientID = self.getui_patientID()
        unzipPath = self.m_editUnzipPath.text()
        maskRoot = os.path.join(unzipPath, self.getui_patientID(), "02_SAVE", "01_MASK")
        self.m_reconStomach.APPath = os.path.join(maskRoot, "Mask_AP")
        self.m_reconStomach.PPPath = os.path.join(maskRoot, "Mask_PP")
        self.m_reconStomach.IntermediateDataPath = self.m_outputPath

        self.m_mediator.load_userdata()
        userData = self._get_userdata()
        dataInst = self.get_data()
        result = self.m_reconStomach.init(dataInst.OptionInfo, userData)
        return result
    ...
    def _cliked_recon_get_sliceID(self):
        ...
        def _manually_detect_navel():
            ...
            def _generate_skin_png(self):
                generateSkinScreenshot.generateSkinPng(os.path.join(PPPath, "Skin.nii.gz"))
