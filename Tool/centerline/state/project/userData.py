...
class CUserData :
    ...
    def blender_process_load(blenderExe : str, blenderFullPath : str) :
        ...
        os.system(cmd)
    @staticmethod
    def blender_process(blenderExe : str, scriptFullPath : str, patientID : str, funcMode : str, optionFullPath : str, stlPath : str, outputPath : str, saveName : str, bBackground : bool = False) :
        '''
        Desc
        ...
        if bBackground == False :
            cmd = f"{blenderExe} --python {scriptFullPath} -- --optionFullPath {optionFullPath} --stlPath {stlPath} --outputPath {outputPath} --saveName {saveName} --funcMode {funcMode} --patientID {patientID}"
        else :
            cmd = f"{blenderExe} -b --python {scriptFullPath} -- --optionFullPath {optionFullPath} --stlPath {stlPath} --outputPath {outputPath} --saveName {saveName} --funcMode {funcMode} --patientID {patientID}"
        os.system(cmd)
    @staticmethod
    def blender_process_load_script(blenderExe : str, blenderFullPath : str, scriptFullPath : str, patientID : str, funcMode : str, optionFullPath : str, stlPath : str, outputPath : str, saveName : str, bBackground : bool = False) :
        '''
        Desc
        ...
        saveName = f"{saveName}.blend"
        if bBackground == False :
            cmd = f"{blenderExe} {blenderFullPath} --python {scriptFullPath} -- --optionFullPath {optionFullPath} --stlPath {stlPath} --outputPath {outputPath} --saveName {saveName} --funcMode {funcMode} --patientID {patientID}"
        else :
            cmd = f"{blenderExe} -b {blenderFullPath} --python {scriptFullPath} -- --optionFullPath {optionFullPath} --stlPath {stlPath} --outputPath {outputPath} --saveName {saveName} --funcMode {funcMode} --patientID {patientID}"
        os.system(cmd)
