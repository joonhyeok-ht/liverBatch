...
class CCommandColonVesselCutting :
    def __init__(self, mediator : componentSelectionCL.CComDrag, dataInst : data.CData) :
        ...
        self.m_inputColonVesselNode = None
    def clear(self) :
    ...
    def Data(self)  -> data.CData :
        return self.m_data
    @property
    def InputColonVesselNode(self) -> CColonVesselNode:
        return self.m_inputColonVesselNode
    @InputColonVesselNode.setter
    def InputColonVesselNode(self, inputColonVesselNode : CColonVesselNode) :
        self.m_inputColonVesselNode = inputColonVesselNode
class CCommandColonVesslCuttingTP(CCommandColonVesselCutting) :
    def __init__(self, mediator : componentSelectionCL.CComDrag, dataInst : data.CData) :
    ...
    def process(self) -> bool :
        if self.InputColonVesselNode is None :
            return False
        ...
        wholeVesselKey = self.InputColonVesselNode.m_subVesselKey
        wholeVesselObj = self.Data.find_obj_by_key(wholeVesselKey)
        self.m_undoVessel = wholeVesselObj.PolyData

        self.m_mediator.pushback_tp(self.InputTPPos)
        wholeVesselObj.PolyData = self.InputVessel
        return True
    def process_undo(self) :
        wholeVesselKey = self.InputColonVesselNode.m_subVesselKey
        wholeVesselObj = self.Data.find_obj_by_key(wholeVesselKey)
        
        wholeVesselObj.PolyData = self.m_undoVessel
        self.m_mediator.popback_tp()

        self.m_mediator.setui_lv_vessel_selection(self.InputColonVesselNode)
        self.m_mediator.command_selection_vessel(self.InputColonVesselNode)
...
class CCommandColonVesselCuttingCut(CCommandColonVesselCutting) :
    def __init__(self, mediator : componentSelectionCL.CComDrag, dataInst : data.CData) :
        super().__init__(mediator, dataInst)
        ...
        self.m_undoColonVesselNode = None
    def clear(self) :
        self.m_inputWholeVessel = None
        self.m_inputSubVessel = None
        self.m_undoVessel = None
    ...
    def process(self) -> bool :
        ...
        wholeVesselObj.PolyData = self.InputWholeVessel
        nowSubVesselID = self.m_mediator.getui_lv_vessel_node_count()
        ...
        subVesselObj.PolyData = self.InputSubVessel
        self.Data.add_vtk_obj(subVesselObj)

        ret = self.m_mediator.find_list_clID(subVesselObj.PolyData)
        if ret is None :
        ...
        self.m_mediator.setui_lv_vessel_selection(node)
        self.m_mediator.command_selection_vessel(node)
        self.m_undoColonVesselNode = node
    ...
    def process_undo(self) :
        ...
        node = self.m_undoColonVesselNode
        self.App.remove_key(node.m_subVesselKey)
...
class CComColonVesselCutting(componentSelectionCL.CComDrag) :
    ...
    def get_vessel_node_count(self) -> int :
        return self.getui_lv_vessel_node_count()
    def get_vessel_node(self, inx : int) -> CColonVesselNode :
        return self.getui_lv_vessel_node(inx)
    def pushback_tp(self, pos : np.ndarray) -> str :
        ...
        color = CComColonVesselCutting.s_tpColor
    ...
    def command_knife_vessel(self, startMx, startMy, endMx, endMy) :
        ...
        rootNode = self.getui_lv_vessel_selection()
        if rootNode is None :
            QMessageBox.information(self.App, "Alarm", "리스트 박스에서 혈관을 선택하세요")
            return

        # rootNode = self.getui_lv_vessel_node(0)
        ...
        if cmd.get_output_polydata_count() == 1 :
            ...
            undoCmd = CCommandColonVesslCuttingTP(self, dataInst)
            undoCmd.InputColonVesselNode = rootNode
        ...
        if cmd.get_output_polydata_count() > 1 :
            ...
            undoCmd = CCommandColonVesselCuttingCut(self, dataInst)
            undoCmd.InputColonVesselNode = rootNode
    ...
    def command_label_name(self, labelName) :
        self.setui_lv_vessel_selection_name(labelName)
    def command_selection_vessel(self, node : CColonVesselNode) :
    ...
    # ui setting
    def setui_lv_vessel_add_node(self, node : CColonVesselNode) :
    ...
    def setui_lv_vessel_remove_node(self, node : CColonVesselNode) :
        self.InputUILVVessel.blockSignals(True)
        ...
        self.InputUILVVessel.blockSignals(False)
    def setui_lv_vessel_selection(self, targetNode : CColonVesselNode) :
        count = self.InputUILVVessel.count()
        for i in range(count):
            ...
            if node == targetNode : 
                ...
                break
    def setui_lv_vessel_selection_name(self, label : str) :
        ...
        if item is not None:
            ...
            if node is not None:
                node.m_name = label
    def setui_lv_vessel_clear_selection(self) :
        self.InputUILVVessel.clearSelection()
    
    def getui_lv_vessel_node_count(self) -> int :
        return self.InputUILVVessel.count()
    def getui_lv_vessel_node(self, inx : int) -> CColonVesselNode :
        item = self.InputUILVVessel.item(inx)
        if item is not None :
            return item.data(Qt.UserRole)
        return None
    def getui_lv_vessel_selection(self) -> CColonVesselNode :
        item = self.InputUILVVessel.currentItem()
        if item is not None:
            return item.data(Qt.UserRole)
        return None
