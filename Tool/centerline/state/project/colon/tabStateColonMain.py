...
class CTabStateColonMain(tabState.CTabState) :
    ...
    def clicked_mouse_rb(self, clickX, clickY) :
        if self.m_stateSelCell == 0 : 
            return
        if self.get_render_entity_count() == 0 :
            return
        entity = self.get_render_entity(0)
        polyData = entity.PolyData
        entity = self.get_render_entity(1)
        entity.PolyData = polyData
    ...
    def _on_btn_resampling(self) :
        ...
        if self.getui_patientID() == "" :
            msg = "not selection patientID"
        if self.getui_output_path() == "" :
            msg = "not setting output path"

        if msg == "" :
            userData = self.m_mediator.ReconUserData
            if userData is not None :
                userData.resampling_mask(self.getui_patientID(), self.getui_output_path())
            msg = "completed resampling"
        
        QMessageBox.information(self.m_mediator, "Alarm", msg)
    ...
    def __set_selcellstate(self, state : int) :
        ...
        if state == 0 :
            ...
            entity = tabState.CRenderEntity()
            entity.Color = algLinearMath.CScoMath.to_vec3([1.0, 0.0, 0.0])
            self.add_render_entity(entity)
            entity = tabState.CRenderEntity()
            entity.Color = algLinearMath.CScoMath.to_vec3([0.0, 1.0, 0.0])
            self.add_render_entity(entity)
        self.m_mediator.update_viewer()
