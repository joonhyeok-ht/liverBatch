...
class CTabStateColonMerge(tabState.CTabState) : 
    ...
    def process_init(self) :
        ...
        # colon, rectum mask ref
        # self.m_mediator.ref_key_type(userDataColon.CUserDataColon.s_colonMaskType)

        # colon, rectum centerline ref
        ctCLInfoInx = ctColonRes.CLInfoInx
        if ctCLInfoInx >= 0 :
            self.m_mediator.ref_key_type_groupID(data.CData.s_skelTypeCenterline, ctCLInfoInx)
        mrCLInfoInx = mrColonRes.CLInfoInx
        if mrCLInfoInx >= 0 :
            self.m_mediator.ref_key_type_groupID(data.CData.s_skelTypeCenterline, mrCLInfoInx)
