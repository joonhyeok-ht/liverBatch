...
class CSkeleton :
    ...
    def init_with_vtk_skel_info(self, vtkSkelInfo : list) :
        ...
        self.rebuild_centerline_related_data()
    def init_with_centerline_list(self, listCL : list) :
        self.clear()
        self.m_topology = None

        centerCnt = len(listCL)
        for i in range(0, centerCnt) :
            srcCL = listCL[i]
            centerline = CSkeletonCenterline(i)
            centerline.Vertex = srcCL.Vertex.copy()
            centerline.Radius = srcCL.Radius.copy()
            self.m_listCenterline.append(centerline)
        self.rebuild_centerline_related_data()
    ...
    def find_ancestor_centerline_by_centerline_id(self, centerlineID : int) -> list :
        ...
        if len(retList) == 0 :
            return None
        return retList
    def is_ancestor(self, ancestorCLID : int, clID : int) -> bool :
        if len(self.m_listTree) == 0 :
            return False
        
        centerline = self.get_centerline(clID)
        treeID = centerline.TreeID
        treeNode = self.m_listTree[treeID]
