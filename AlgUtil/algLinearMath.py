...
class CScoMath :
    ...
    @staticmethod
    def create_plane(v0 : np.ndarray, v1 : np.ndarray, v2 : np.ndarray) -> np.ndarray :
        e0 = v1 - v0
        e1 = v2 - v0
        ...
        return retPlane
    @staticmethod
    def create_plane_normal_point(normal : np.ndarray, point : np.ndarray) -> np.ndarray :
    ...
    @staticmethod
    def transform_plane(mat4 : np.ndarray, plane : np.ndarray, pt : np.ndarray) -> np.ndarray :
        '''
        desc
            - plane을 해당 pt를 기준으로 transform 시킨다.
        input
            - mat4 : 4x4 행렬 
            - pt : plane상에 존재하는 point 
        '''
        transPt = CScoMath.mul_mat4_vec3(mat4, pt)
        rotMat = mat4.copy()
        rotMat[:3, 3] = 0.0

        normalTransMat = CScoMath.inv_mat4(rotMat)
        ...
        # voxel offset(d) 계산
        d_vox = -np.dot(n_vox, transPt[0])

        # 최종 plane 반환
        return np.hstack([n_vox, d_vox])
    @staticmethod
    def transform_normal(mat4 : np.ndarray, normal : np.ndarray) -> np.ndarray :
        rotMat = mat4.copy()
        rotMat[:3, 3] = 0.0
        ...
        n_vox = normalTransMat[:3, :3] @ normal.reshape(-1)
        n_vox /= np.linalg.norm(n_vox)

        return n_vox.reshape(-1, 3)
