...
class CBSMImportSTL(blenderOption.CBlenderScriptBase) :
    '''
    param
    ...
    def process(self) -> bool :
        ...
        for stlFullPath in listStlFullPath :
            stlFile = os.path.basename(stlFullPath)
            stlName = stlFile.split('.')[0]
            ext = stlFile.split('.')[-1]
            if ext != "stl" :
                continue
            # 기존것은 삭제   
            if stlName in bpy.data.objects :
                blenderOption.CBlenderScriptUtil.delete_object(stlName)
...
if __name__=='__main__' :
    ...
    if "--" in args :
        ...
        if optionFullPath is None :
            ...
            inst = CBSMImportSTL(optionFullPath)
            inst.process()
